...
def handle(event, context):
    ...
    result_avg_rating_per_month = db.query(query=query_avg_rating_per_month)
    result_count_per_month = db.query(query=query_count_per_month)
    result_sum_per_month = db.query(query=query_sum_per_month)
    result_total_users = db.query(query=query_total_users)

    total_sum = 0.0

    for result_sum in result_sum_per_month:
        result_sum["total_month"] = float(result_sum["total_month"]) * 0.34
